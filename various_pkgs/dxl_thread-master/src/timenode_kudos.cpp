...
//static struct timespec curr_time;
  ...
  while(is_run) //1하면 종료가 잘 안됨. is run으로
  {
    ...
    if (kudos == 0) { //k
      process0();
      if(index_1 == index_k) {
        ROS_WARN("is run index : %d == %d", index_1, index_k);
        is_run  = false;
      }
    }
    else if (kudos == 1) { //u
...
void process0(void) { //k

    End_point goal_arr[index_k];
    struct End_point target;

    EP_goal.x = 0;
    EP_goal.y = 0.2;  //start point
    goal_arr[0] = EP_goal;
...
void process1(void) { //u
    ...
    if (flag) {
      if (t <= T) {
         ROS_INFO("flag = true");
         ROS_INFO("x:%lf, y:%lf",E.x,E.y);
         EP_goal.x = 0.040000;//0.0; //0.049742;//
         EP_goal.y = 0.1;
    ...
    if (!flag) {
      if (t <= T) {
        ...
        target.x = 0.5*0.08*cos(PI*(t/T));
        target.y = 0.08*sin(PI*(t/T))+0.1;

//        target.x = 0.05*cos(2*PI*(t/T));
//        target.y = 0.05*sin(2*PI*(t/T))+0.1;

        ROS_WARN("tx:%lf, ty:%lf",target.x,target.y);
...
//      target.x = 0.05*cos(PI*(t/T));
//      target.y = 0.9*(0.05*sin(PI*(t/T))+0.1);
...
void process3(void) { //o
  ...
  if (!flag) {
    if (t <= T) {
      ...
      target.x = 0.05*cos(2*PI*(t/T));
      target.y = 0.7*(0.05*sin(2*PI*(t/T)))+0.1;
//      target.x = 0.05*cos(2*PI*(t/T));
//      target.y = 0.05*sin(2*PI*(t/T))+0.1;

      ROS_WARN("000000000000tx:%lf, ty:%lf",target.x,target.y);
...
void process4(void) { //s
  ...
  if (flag) {
    if (t <= T) {
       ROS_INFO("flag = true");
       ROS_INFO("x:%lf, y:%lf",E.x,E.y);
       EP_goal.x = 0.0;//0.049661;//0.0;
       EP_goal.y = 0.05;//0.100360;
      ...
      if (t3 <= T) {
        ...
        target.x =  p_pos.x + (EP_goal.x - p_pos.x )*0.5*(1 - cos(PI* t3/T));
        target.y =  p_pos.y + (EP_goal.y - p_pos.y )*0.5*(1 - cos(PI* t3/T));
  ...
  if ((!flag) && (!flag2)) {
    if (t4 <= T) {
      ...
      target.x = t4/T*0.1;
      target.y = 0.05*sin(2*PI*(t4/T)) + 0.1;
